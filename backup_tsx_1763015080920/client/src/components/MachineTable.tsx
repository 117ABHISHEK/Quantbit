...
interface MachineTableProps {
  ...
  onViewLogs: (machineId: string) => void;
}

export function MachineTable({ machines, isLoading, onEdit, onDelete, onViewLogs }: MachineTableProps) {
  if (isLoading) {
    return (
        ...
        <Table>
          ...
          <TableBody>
            {machines.map((machine) => (
              <TableRow key={machine.id} className="hover-elevate" data-testid={`row-machine-${machine.id}`}>
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    <Button
                      size="icon"
                      variant="ghost"
                      onClick={() => onViewLogs(machine.id)}
                      data-testid={`button-view-logs-${machine.id}`}
                    >
