// Lightweight converter: reads all .tsx files under client/src and writes .jsx counterparts
// Usage: node ./scripts/convert-tsx-to-jsx.js
// Requires the project to have `typescript` installed (devDependency present).
...
function convertFile(filePath) {
  ...
  const transpiled = ts.transpileModule(src, {
    compilerOptions: {
      jsx: ts.JsxEmit.Preserve, // keep JSX syntax
  ...
  // post-process: remove "export {}" artifacts sometimes emitted and ensure .jsx extension
  let out = transpiled.outputText;

  // remove TypeScript import type lines that may remain as empty imports
  out = out.replace(/^import\s+type\s+.*;\r?\n/gm, '');

  // write .jsx file next to original
...
function main() {
  ...
  // create a simple backup folder in project root
  ...
  console.log('Next: run your dev server and fix any runtime errors from differences in typing or generics.');
}
